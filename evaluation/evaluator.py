...
def inference_on_dataset(model, data_loader, evaluator, flip_test=False):
    ...
    np.save("/data1/fast-reid/tools/output/extracted_features.npy", extracted_features)
    np.save("/data1/fast-reid/tools/output/extracted_targets.npy", extracted_targets)
...
def inference_retrieval(model, data_loader, evaluator):
    """
    Run model on the data_loader and evaluate the metrics with evaluator.
    The model will be used in eval mode.
    Args:
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            start_compute_time = time.perf_counter()
            outputs = model(inputs, retrival=True)
