...
from .common import CommDataset, ViFiDataset
...
_root = os.getenv("FASTREID_DATASETS", "/data1/fast-reid/datasets")
...
def _train_loader_from_config(cfg, *, train_set=None, transforms=None, sampler=None, **kwargs):
    ...
    if train_set is None:
        ...
        train_set = ViFiDataset(train_items, transforms, relabel=True)
...
def _test_loader_from_config(cfg, *, dataset_name=None, test_set=None, num_query=0, transforms=None, **kwargs):
    ...
    if test_set is None:
        ...
        test_set = ViFiDataset(test_items, transforms, relabel=False)
...
def fast_batch_collator(batched_inputs):
    ...
    elem = batched_inputs[0]
    if isinstance(elem, torch.Tensor):
        # Check and ensure all tensors have the same shape
        tensor_shapes = [x.size() for x in batched_inputs]
        if not all(shape == tensor_shapes[0] for shape in tensor_shapes):
            for shape in tensor_shapes:
                print(shape)
            raise ValueError("All tensors in the batch must have the same shape")
